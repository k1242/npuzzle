// flip.js - Descent engine for triple reduction
(function(global) {
  'use strict';
  ...
  // Main API: reduce components
  function reduceComponents(components, n, { flipLim = 1_000_000 } = {}) {
    ...
    for (let i = 0; i < result.length; i += 3) {
      if (result[i] !== 0n && result[i + 1] !== 0n && result[i + 2] !== 0n) {
        filtered.push(result[i], result[i + 1], result[i + 2]);
      }
    }
    
    return flatToComponents(filtered, n);
  }
